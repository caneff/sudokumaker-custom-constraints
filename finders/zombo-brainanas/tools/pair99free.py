...
usage: pair99.py shard nshards [limit] [stall] -> hunt/pair99free/full_r9c7_r9c9_<kind>.json"""
...
OUT = Z + "hunt/pair99free/"; os.makedirs(OUT, exist_ok=True)
...
for i, (name, a, sh) in enumerate(cases):
    ...
    found = zb.solve_valid(circles={a[0]: None, a[1]: None}, seed=i, limit=limit, min_pockets=1, objective=True,
                           log=log, stall=stall, stop_at=99, fix_shade=sh, bonus=BONUS)
    if found is None:
