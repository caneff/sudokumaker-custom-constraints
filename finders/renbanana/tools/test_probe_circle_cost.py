...
only ties its digit to a size already in the model. A per-circle closure+rank
system (81 bools, 81 ranks and their links, per circle) left a 12-circle run
without a first solution in 8 minutes (#401).
...
def test_twelve_circles_add_no_variables_and_one_constraint_each():
    bare_vars, bare_cons = size(pcp.build([])[0])
    vars12, cons12 = size(pcp.build(TWELVE)[0])
