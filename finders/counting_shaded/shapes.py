...
1-8, distinct within every row, column and box, the shaded cells form one
...
solutions. Hill climbing toggles shaded cells and keeps a move that leaves the
...
def givens(shape):
    ...
    Inadmissible covers: an isolated shaded cell, a repeated given in a house, and a
    shaded cell region that is not orthogonally connected."""
...
REQUIRE_EIGHT = False  # set True to demand a shaded cell showing 8
FORCE = set()  # cell indices that must be shaded cells
BAN = set()  # cell indices that must not be shaded cells
...
def climb(rng, deadline, cap, min_shaded):
    shape = seed_shape(rng, min_shaded)
    g = givens(shape)
...
def main():
    ...
    while time.monotonic() - t0 < a.seconds:
        ...
        print(
            f"climb {climbs}: shaded cells={len(shape)} solutions={score}{'+' if score >= a.cap else ''} "
