...
the givens. One shaded cell must hold 8. Uniqueness is plain sudoku uniqueness of
...
see 2026-09-15-counting-shaded-all-visible.md):
  ...
  check   sudoku with the shaded cell digits given and the master grid forbidden,
...
class Master:
    def __init__(self, rng, min_shaded):
        ...
        # on[c][d]: cell c is a shaded cell holding digit d+1
        self.on = on = {}
        for cell in CELLS:
            on[cell] = []
            for d in range(9):
        ...
        m.add_bool_or([on[cell][7] for cell in CELLS])  # an 8-cell (already implied)
        self.size = sum(g.values())
        m.add(self.size >= min_shaded)
        for band in BANDS:  # ≤ 1 unshaded row per band, column per stack
            rows_hit = []
            cols_hit = []
            for i in band:
        ...
        # of links with no shaded cell in either row can rotate: a second solution.
        ...
        # unshaded link. Same for two columns of one stack.
        ...
        # a and b on those 6 cells keeps every house: one must be a shaded cell. Same
        ...
        for transpose in (False, True):
            ...
            for band in BANDS:
                ...
                # and boxes keep their sets, so one of the 6 cells is a shaded cell.
                for j0 in range(N):
                    for j1 in range(j0 + 1, N):
                        if j0 // 3 == j1 // 3:
                            continue
                        shaded = [g[at(i, j)] for i in band for j in (j0, j1)]
                        for a in range(9):
                            for b in range(a + 1, 9):
...
def main():
    ...
    master = Master(rng, a.min_shaded)
    end = "time"
    while time.monotonic() < deadline:
        ...
        if Y is None:
            ...
            print(
                f"unique: {len(shape)} shaded cells verified={ok} cuts={master.cuts} t={rec['elapsed_s']}s",
                flush=True,
            )
