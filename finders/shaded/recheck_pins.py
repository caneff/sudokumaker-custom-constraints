...
of a count, rather than as pairwise != between shaded cells. Positive controls run
...
def model(force, ban):
    ...
    # v[i]: the given on a shaded cell, or a dummy unique to the cell otherwise.
...
def main():
    ...
    for label, force, ban in (
        ...
        print(
            f"  {label}: {st}"
            + (f", {len(shape)} shaded cells, checks out: {ok}" if shape else "")
        )
