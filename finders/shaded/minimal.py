...
8-cell is always visible. Greedy removal to a minimal set: no single clue can
go without a second digit solution appearing. Hidden shaded cells stay free.
...
def random_grid(rng, min_shaded):
    m, x, g = base_model()
    eight = rng.choice(
        [
            cell
    ...
    shaded = {cell for cell in CELLS if s.value(g[cell])}
    return digits, shaded, eight


def minimise(rng, digits, shaded, eight, stats):
    m, x, g = base_model()
    clue_lit = {}
    for cell in CELLS:
    ...
    for cell in shaded:
        ...
        clue_lit[("shaded cell", cell)] = lit
    m.add(g[eight] == 1)  # the 8-cell is visible: fixed, not removable
    m.add_bool_or(
...
def main():
    ...
    ap.add_argument("--min-shaded", type=int, default=9)
    ...
    for i in range(a.grids):
        t0 = time.monotonic()
        grid = random_grid(rng, a.min_shaded)
        if grid is None:
            print(f"grid {i}: generation failed", flush=True)
            continue
        digits, shaded, eight = grid
        stats = {"checks": 0, "check_s": 0.0}
        clues = minimise(rng, digits, shaded, eight, stats)
        ok = verify(digits, eight, clues)
        n_digit = sum(k == "digit" for k, _ in clues)
        n_shaded = len(clues) - n_digit + 1  # + the fixed 8-cell
        total = n_digit + n_shaded
        rec = {
            "grid": i,
            "seed": a.seed,
            "total": total,
            "digits_given": n_digit,
            "shaded_visible": n_shaded,
            "shaded_in_solution": len(shaded),
            "eight": eight,
            "verified": ok,
            "checks": stats["checks"],
            ...
            "clues": [[k, list(cell)] for k, cell in clues]
            + [["shaded cell", list(eight)]],
        }
        with (a.out / "progress.jsonl").open("a") as f:
            f.write(json.dumps(rec) + "\n")
        print(
            f"grid {i}: total {total} (digits {n_digit}, shaded cells {n_shaded}/{len(shaded)}) "
