...
maximises the shaded cell count under a short time limit from random digit hints,
and checks the shape with the bitmask counter (cap 2). A non-unique shape's
second solution Y becomes a cut (some shaded cell must disagree with Y) and the same
    ...
    while time.monotonic() - t0 < a.seconds:
        ...
        for _ in range(a.rounds):
            ...
            if n == 1:
                ...
                print(
                    f"solve {solves}: UNIQUE {len(shape)} shaded cells{'' if new else ' (dup)'}; "
            ...
            model.cut(Y)
            print(
                f"solve {solves}: {len(shape)} shaded cells, not unique "
