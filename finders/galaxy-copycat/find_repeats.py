...
D = Path("docs/research/2026-09-14-galaxy-copycat/boards")
base = json.loads((D / "board14-four-lines.json").read_text())
seed = json.loads((D / "board14-forced.json").read_text())
with (D / "board14-pair4.lines.jsonl").open() as f:
    lines = {r["cells"]: r for r in map(json.loads, f)}
with (D / "board14-pair4.pairs.jsonl").open() as f:
    pairs = [json.loads(l) for l in f if l.startswith("{")]
with (D / "board14-pair4.rank.jsonl").open() as f:
    rank = {
        (r["X"], r["Y"]): r
        for r in map(json.loads, (l for l in f if l.startswith("{")))
    }
...
t0 = time.time()
for p in pairs:
    if tuple(lines[p["X"]]["struct"]) != (2, 2) or tuple(lines[p["Y"]]["struct"]) != (
        2,
        2,
    ):
        continue
    ...
    for ms in sorted(shared):
        setup = {
            **seed,
            "lines": dict(base["lines"], X=p["X"].split("-"), Y=p["Y"].split("-")),
        ...
        out.append(
            {
                ...
                "Y": p["Y"],
                "repeat_msets": ok,
                "all_msets": sorted(msets(p["X"]) & msets(p["Y"])),
                "new_fixed": r.get("new_fixed"),
            }
        )
