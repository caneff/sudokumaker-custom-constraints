"""Ghosts, connected: search the shape inside a fixed solved grid. BROKEN SAMPLER.

    uv run finders/ghosts/gridfirst.py --seconds 300 --min-ghosts 20
...
class out of 5.47 billion, not the space. The ghost-count constraint is not
invariant under those symmetries, so results vary just enough to look like
sampling.

It is kept as a recorded dead end. Its reported ceiling of 9 ghosts over 38,064
...
`docs/research/2026-09-16-ghosts-connected-decision-log.md`, entries 3 and 4.
...
fix a solved grid first, then a ghost set S only has to satisfy

    grid[i] == |NEIGH[i] & S|   for every i in S

with S orthogonally connected. Admissibility is then free (the grid is a valid
sudoku, so two ghosts in a house cannot show the same digit), and every hit is
...
def shapes_for_grid(grid, min_ghosts, max_ghosts, seconds, workers, limit):
    """Connected ghost sets this grid describes, newest-first, up to `limit`."""
    m = cp_model.CpModel()
...
def main():
    ...
    ap.add_argument("--min-ghosts", type=int, default=18)
    ap.add_argument("--max-ghosts", type=int, default=40)
    ...
    while time.monotonic() - t0 < a.seconds:
        ...
        found = shapes_for_grid(
            grid, a.min_ghosts, a.max_ghosts, left, a.workers, a.per_grid
        )
        shaped += len(found)
        for shape in found:
            gv = givens(shape)
            ...
            if n == 1:
                unique += 1
                hits.append(
                    {"ghosts": len(shape), "shape": sorted(shape), "grid": grid}
                )
                print(f"UNIQUE {len(shape)} ghosts: {sorted(shape)}")
                if a.out:
