"""Merge the census output and annotate every configuration with its solution count.
    ...
    uv run finders/ghosts/batch_count.py .scratch/ghosts/*/configurations.jsonl \
        --out docs/research/ghosts/connected-corpus.jsonl

Nothing is rejected. A configuration with 40,000 solutions is as much a part of
the corpus as a unique one -- it is a valid connected all-visible ghost grid
...
The counter runs over the givens alone, which is correct here: with every ghost
visible the ghost rule is redundant, since all solutions agree on the ghost
cells, so the board is a plain sudoku whose givens are the ghost digits. See
docs/research/2026-09-16-ghosts-connected.md.
"""
...
def givens_of(shape):
    """{cell: ghost-neighbour count} -- the givens the shape puts on the board."""
    s = set(shape)
...
def main():
    ...
    print(f"{'ghosts':>7} {'unique':>7} {'2..cap':>7} {'cap+':>7}")
    ...
    if fewest:
        print(
            ...
            f"at {fewest['ghosts']} ghosts, shape {fewest['shape']}"
        )
    if a.out:
        a.out.parent.mkdir(parents=True, exist_ok=True)
        with a.out.open("w") as fh:
            for row in sorted(rows, key=lambda r: (r["ghosts"], r["shape"])):
