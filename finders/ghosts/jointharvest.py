"""Ghosts, all visible and connected: enumerate joint seeds, keep the unique ones.

    uv run finders/ghosts/jointharvest.py --seconds 1800 --min-ghosts 20 --out DIR
...
is the joint one -- digits, ghosts, the all-visible link, flow connectivity --
so every shape it returns is connected, admissible and solvable by
construction. `joint.py --maximize` proves the ceiling is 26 ghosts.
...
def main():
    ...
    while time.monotonic() - t0 < a.seconds:
        ...
        m, _v, g = build(a.min_ghosts, a.max_ghosts, maximize=False, weights=weights)
        s = cp_model.CpSolver()
        ...
        for cand, how in cands:
            ...
            seen.add(key)
            stats["unique"] += 1
            row = {"ghosts": len(cand), "shape": sorted(cand), "from": how}
            with (a.out / "connected.jsonl").open("a") as fh:
                fh.write(json.dumps(row) + "\n")
            print(f"UNIQUE {len(cand)} ghosts via {how}: {sorted(cand)}", flush=True)
