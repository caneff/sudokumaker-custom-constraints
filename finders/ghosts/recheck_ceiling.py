"""Second opinion on the 26-ghost ceiling, sharing no encoding with joint.py.

    uv run finders/ghosts/recheck_ceiling.py --seconds 300
    uv run finders/ghosts/recheck_ceiling.py --exactly 27   # expect INFEASIBLE

`joint.py` says the largest connected all-visible ghost set is 26, OPTIMAL.
...
* the all-visible link as `cnt[i] == d` enforced on the pair (ghost, digit d),
  not as an equality between a digit variable and a sum;
* connectivity as a rooted spanning tree with level labels -- every ghost is
...
def add_tree_connectivity(m, g):
    """Rooted spanning tree: one root, every other ghost has a parent below it."""
    ...
    for i in range(81):
        ...
        for j in ORTH[i]:
            ...
            parents.append(p)
        # every ghost is the root or has exactly one parent; a non-ghost neither
...
def main():
    ...
    print(f"  ghosts {len(shape)}  flood-fill connected: {connected(shape)}")
    ok = all(grid[i] == sum(j in shape for j in NEIGH[i]) for i in shape)
    print(f"  every ghost digit equals its neighbour count: {ok}")
    for r in range(9):
        print(
            "  "
            + " ".join(
