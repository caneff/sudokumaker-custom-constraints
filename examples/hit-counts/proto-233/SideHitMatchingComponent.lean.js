...
// of them, so most calls arrive on a state this component has already filtered.
// Hash the side's candidate masks and record the hash of the state this
// component itself produced; an entry hash equal to it means nothing has moved
// since, so there is nothing to find. A backtrack restores candidates and
// changes the hash, so the deductions are made again on the way back down.
function signature (puzzle, cells) {
  let h = 0
  for (const cell of cells) h = (Math.imul(h, 31) + puzzle.getCandidatesBitMask(cell)) | 0
  return h
}

function * update (instance, puzzle) {
  const { clues, lines } = instance
  if (instance.cells === undefined) instance.cells = getAffectedCells(clues, lines)
  const sig = signature(puzzle, instance.cells)
  ...
  instance.sig = signature(puzzle, instance.cells)
}
