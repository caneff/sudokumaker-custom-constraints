...
def make_grid(rng):
    rows_=[g*3+r for g in rng.sample(range(3),3) for r in rng.sample(range(3),3)]
    cols_=[g*3+c for g in rng.sample(range(3),3) for c in rng.sample(range(3),3)]
...
def unique(clue, active, givens):
    ...
    for (r,c),v in s1.items():
        b=m.NewBoolVar(f"d{r}{c}"); m.Add(x[r,c]!=v).OnlyEnforceIf(b); m.Add(x[r,c]==v).OnlyEnforceIf(b.Not()); lits.append(b)
    m.AddBoolOr(lits)
...
for seed in range(101,113):
    ...
    cells_all=[(r,c) for r in range(9) for c in range(9)]; rng.shuffle(cells_all)
    if not unique(clue,active,givens):
        for cell in cells_all:
            givens[cell]=grid[cell[0]][cell[1]]
            if unique(clue,active,givens): break
    # carve givens (all clues kept) to minimize
    for cell in list(givens.keys()):
        v=givens.pop(cell)
        if not unique(clue,active,givens): givens[cell]=v
    n=len(givens)
