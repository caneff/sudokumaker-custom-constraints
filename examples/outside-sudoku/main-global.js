// Outside Sudoku, GLOBAL variant. No groups arrive: build every frame line
// from the board size — an interior nw = W-2 wide and nh = H-2 tall, ringed by
// one clue cell per row and per column. A left or right clue reads one interior
// row, so it has nw cells and there are nh such lines; a top or bottom clue
// reads one interior column, so it has nh cells and there are nw of them.
