...
def load_gen(n):
    ...
    givens = {(int(r), int(c)): v for k, v in g["givens"].items() for r, c in [k.split(",")]}
...
def swap_to_original(doc):
    ...
    sk["definition"]["components"] = [{
        "type": "code", "name": "CustomSkyscraperLineComponent",
...
def constraint_code(doc):
    sk = next(c for c in doc["puzzle"]["constraints"]
              if c.get("definition", {}).get("name") == "Skyscraper Lines")
    return {"backend": sk["definition"]["backend"]["code"],
...
def blanked(doc):
    ...
    d = copy.deepcopy(doc)
    sk = next(c for c in d["puzzle"]["constraints"]
              if c.get("definition", {}).get("name") == "Skyscraper Lines")
...
if __name__ == "__main__":
    ...
    assert blanked(improved) == blanked(original), "frames differ beyond the constraint code"
    assert constraint_code(improved) != constraint_code(original), "code was not swapped"
    assert original["puzzle"]["cells"] == improved["puzzle"]["cells"], "cells differ"

    link = "https://sudokumaker.app/?puzzle=" + LZString.compressToEncodedURIComponent(json.dumps(original))
    ...
    print(f"wrote PUZZLE_LINK_{n}x{n}_original.txt ({len(link)} chars) "
