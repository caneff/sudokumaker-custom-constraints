...
//! Numbered Rooms Lines: Numbered Rooms on ANY drawn line — a diagonal, a bent
//! path, a partial row. Same rule as NumberedRoomsComponent; the difference is
//! the `distinct` flag. Two prunes assume the line cells hold distinct digits
//! (target ≠ k for k > 1; a solved clue sits in exactly one line cell). They
//! run only when the main code proves the cells see each other. On any other
//! line the component keeps the three prunes that hold regardless.
//!
//! A line reads inward from a clue cell. Let O be the clue cell and line[0..m-1] the inside cells,
...
function getAffectedCells (clue, line, distinct) {
  return [clue, ...line]
}

function setParams (instance, clue, line, distinct) {
  instance.clue = clue
  instance.line = line
...
// The k > 1 fact needs distinct line cells, so it is gated on `distinct`.
// Every read is of the pre-pass masks, so the k = 1 case needs no second pass:
// the indexer's own mask is read before this pass prunes it.
function * update (instance, puzzle) {
  const { clue, line, distinct } = instance
  const m = line.length
  ...
  // from every cell at a dead index — the converse of the loop above. Needs
  // distinct cells: with repeats c may sit at a dead index too.
  if (distinct && (clueM & (clueM - 1)) === 0) {
    for (let k = 1, bit = 2; k <= m; k++, bit <<= 1) {
