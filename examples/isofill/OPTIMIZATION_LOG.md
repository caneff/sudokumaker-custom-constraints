...
|---|---|---|---|---|---|
| Capacity deduction (a digit whose reach walk meets fewer than ten cells is a dead branch) | Kept | Real app, stripped grids, median of 3: 35 givens — still no verdict with or without capacity | pairs with reach to close the search, alone insufficient | shipped grid + new 44-clue fixture, app v2026.08.14 | 9e32c8f |
...
| Homeless-digit deduction (a digit with no placed cell needs a ten-cell connected home) (#91) | Rejected — no verdict change and no time change | no number recorded beyond "no verdict change, no time change" | sound, but superseded | recorded at final removal, 35-given verdict | bea6378 (added), 2f8780f (removed — final 35-given verdict recorded) |
| One-pass `update` (read each cell's candidates once per call, build all four deductions' sets from one scan, instead of one scan per digit) | Kept | Halved the app's verdict on the 44-given fixture: 5.7 s → 11.2 s baseline (one-pass reads 5.7 s), same session | never weaker at the fixpoint: 5,000-state differential against the per-digit scan — equal on 4,816, strictly tighter on 184, looser on none; harness asserts the read count | 44-given fixture, same session | cef2afd |
