...
def _link(
    ...
    False drops it, "stale" embeds an older copy. `comment_text` replaces the
    comment outright, for a plain 9x9 board whose sentence is not RULES_PREFIX.
    """
    ...
    if house_gac_renamed:
        comment = NO_RING_RULES_PREFIX + "test rules"  # house-gac is ringless
...
def example(
    ...
    default_link = (
        _link(comment_text=NO_RING_RULES_PREFIX + "test rules")
        if name == "house-gac"
        else _link()
    )
...
if __name__ == "__main__":
    ...
    # plain sentence (#460). The inner-grid sentence there is now the violation.
    ...
    with example(name="house-gac", contents=inner) as (root, _):
        violations = check_tree(root)
        assert any("rules prefix" in v for v in violations), violations
