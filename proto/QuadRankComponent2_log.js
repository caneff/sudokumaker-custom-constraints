...
function * update (instance, puzzle) {
  ...
  if (!globalThis.__qrinit) { globalThis.__qrinit = 1; console.log('[probe] init W=' + W + ' me=' + me + ' n=' + n) }
  ...
  for (let j = 0; j < 4; j++) {
    ...
    for (const d of puzzle.getCandidates(cells[j])) {
      ...
      if (rank - 1 < Ld || rank - 1 > Pd) {
        globalThis.__qrrm = (globalThis.__qrrm || 0) + 1
        if (globalThis.__qrrm <= 40) console.log('[probe] rm cell=' + cells[j] + ' d=' + d + ' me=' + me + ' rank=' + rank)
        yield puzzle.removeCandidateFromCell(d, cells[j])
      }
    }
  }
}
