...
_COMPONENT_FILE = (
    sys.argv[sys.argv.index("--component") + 1]
    if "--component" in sys.argv
    else "QuadRankComponent.js"
)
COMPONENT = pathlib.Path(__file__).with_name(_COMPONENT_FILE).read_text()
...
def row_cages():
    return [{"cells": [r * N + c for c in range(N)], "value": 0} for r in range(N)]


def col_cages():
    return [{"cells": [r * N + c for r in range(N)], "value": 0} for c in range(N)]
    ...
    doc = {
        ...
        "puzzle": {
            ...
            "constraints": [
                ...
                # Rows and columns are NOT implied by the region constraint on a
                # "custom" board: without these the app enforces boxes only, and
                # every #324/#325/#328 board was solved as a box-only puzzle.
                # Same shape framebuild.py uses for the frame boards.
                {
                    "name": "Rows",
                    "type": 301,
                    "cages": row_cages(),
