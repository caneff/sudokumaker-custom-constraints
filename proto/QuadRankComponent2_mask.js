...
function * update (instance, puzzle) {
  ...
  if (!globalThis.__qrmask) {
    globalThis.__qrmask = 1
    const m0 = puzzle.getCandidatesBitMask(allCells[0])
    const cands = Array.from(puzzle.getCandidates(allCells[0]))
    console.log('[probe] cell0 mask=' + m0 + ' cands=' + cands.join('/') + ' value=' + puzzle.getValue(allCells[0]))
    console.log('[probe] loDigit=' + loDigit(m0) + ' hiDigit=' + hiDigit(m0))
  }
  const W = wins.length
