...
// Deduction 4, saturation (#381). Everything above prunes INWARD -- this clue's
// own four cells. But "exactly R-1 windows are strictly below me" is a count
// that can run out, and when it does it constrains every OTHER window:
...
// Either way a digit in another window's cell that would break the settled
// relation is dead. getAffectedCells already returns the whole grid, so the app
// wakes this clue on those cells anyway; only the pruning was missing.
//
...
function * update (instance, puzzle) {
  ...
  // Deduction 4: the count is saturated, so every other window's relation to me
  // is settled. Prune outward.
  const [L0, P0] = count(lo[me], hi[me])
  if (L0 === rank - 1 || P0 === rank - 1) {
    const below = new Array(W).fill(false)
    const maybe = new Array(W).fill(false)
    for (let i = 0; i < W; i++) {
      if (i === me) continue
      if (known[i] === 1) { below[i] = true; maybe[i] = true; continue }
      if (known[i] === -1) continue
      if (hi[i] < lo[me]) below[i] = true
      if (lo[i] < hi[me]) maybe[i] = true
    }
    // Saturated low: nothing outside `below` may be strictly under me.
    // Saturated high: everything inside `maybe` must be strictly under me.
    for (let i = 0; i < W; i++) {
      if (i === me) continue
      const mustNotBeBelow = L0 === rank - 1 && !below[i]
      const mustBeBelow = P0 === rank - 1 && maybe[i]
      if (!mustNotBeBelow && !mustBeBelow) continue
      const w = wins[i]
      for (let j = 0; j < 4; j++) {
        const m = masks[w[j]]
        if ((m & (m - 1)) === 0) continue // already pinned
        for (const d of puzzle.getCandidates(allCells[w[j]])) {
          let uLo = 0; let uHi = 0
          for (let k = 0; k < 4; k++) {
            const mk = masks[w[k]]
            uLo = uLo * 10 + (k === j ? d : loDigit(mk))
            uHi = uHi * 10 + (k === j ? d : hiDigit(mk))
          }
          // uHi < lo[me] forces u strictly below; uLo >= hi[me] forces it not.
          if (mustNotBeBelow && uHi < lo[me]) yield puzzle.removeCandidateFromCell(d, allCells[w[j]])
          else if (mustBeBelow && uLo >= hi[me]) yield puzzle.removeCandidateFromCell(d, allCells[w[j]])
        }
      }
    }
  }
