...
| ISOFILL hard grid (stripped, 32 givens, `puzzle-32.json`, 2026-08-27, 3 reps) | cut only **40.4 s**; walk without allocation (typed `reach`, neighbour lists once) **27.6 s**; + budget flow **26.8 s**. Same grid with a player's correct 2-candidate marks (link 2): 15.2 s → 12.4 s → **7.3 s** | — | the 35-given shipped grid is minimal (every removal breaks uniqueness) and solves in 0.2 s, too fast to rank rules; this grid comes from a CP-SAT sample stripped in the app. Trace: 94% of calls sat under one wrong guess of the digit with no given (4 at the corner), which only cross-digit budget refutes. Component-size rule tried and removed: 44.2 s (no gain) |
