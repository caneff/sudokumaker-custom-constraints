...
| Skyscraper 9×9 (shipped `PUZZLE_LINK.txt`, given-only, 21 active clues, 15 blank) | **unique in 0.3 s** with the joint peak-split `SkyscraperLineComponent` (2026-08-27, 3/3, reps 0.3/0.3/0.3 s, `just time skyscraper --ring-clues`, v2026.08.14-d47fc4b). Before the joint component (per-line DP + pair cap): >300 s (`[timeout]`, 2026-08-27) | — | the joint component pays for itself: the true board went from over the app time limit to under a second — #128 brought it to 2.8 s, the bitmask DP of #134 to 0.3 s |
...
| Skyscraper 9×9, one-1-per-side count in `main.js` (#129) | with the rule / without it, same board both sides, `app-solve.mjs --ring-clues` median of 3 reps: shipped `PUZZLE_LINK.txt` **300 ms / 200 ms**, then 300 ms / 300 ms; timing board (`PUZZLE_LINK_timing.txt`) **3800 ms / 4000 ms**, then 3800 ms / 3500 ms (2026-08-27, v2026.08.14-d47fc4b) | — | kept: a wash. The two boards disagree on sign and the second pair flips the timing board's, so dropping `ExactDigitCountComponent('one 1 on side s', 1, 1, sideClues)` buys nothing — and on the shipped board the whole gap is one 100 ms reporting step. `just time` is blind to a `main.js`-only edit (#151), so these came from the manual backend-swap path the #135 row describes |
| Skyscraper 9×9, at-most-one clue n per side (#135) | timing board (`PUZZLE_LINK_timing.txt`): baseline/candidate 3900 ms / 3800 ms, then 3800 ms / 3800 ms; shipped board (`PUZZLE_LINK.txt`): 300 ms / 300 ms (each a `just time skyscraper --ring-clues` median of 3, 2026-08-27, v2026.08.14-d47fc4b) | — | dropped: `MaxDigitCountComponent('at most one n on side s', maxDigit, 1, sideCells)` added next to the existing exactly-one-1 rule is sound (2000-run harness, 0 violations) and goldens/search-node counts are unchanged (gen_6 8→8, gen_9 762→762, gen_9_timing 6972→6972), but it removed 0 candidates across 125,072 `update` calls — a clue of n needs a fully ascending line, 1/9! per random line, none present in the timing board — and the timing is a wash below the 0.9× bar. `just time`'s `byte_equal` check compares only registered component code, so it read this backend-only `main.js` change as byte-equal and printed a BASELINE-only row; the real numbers above came from `link_swap.replace_constraint_code(..., backend_code=minify_js(main.js))` plus `probe_link.py empty` and `app-solve.mjs` (see #151) |
| Skyscraper 9×9, Régin matching on peak (n) positions (#136) | mock only: gen_9_timing 6972→6972 nodes, gen_9 762→762 nodes; root recovery identical (328 cands/5 passes, 261/4); goldens byte-identical (Phase 1 mock prototype, 2026-08-27, v2026.08.14) | — | dropped: `SkyscraperPeakMatchingComponent` (rows→peak-column Régin matching on digit n, Kuhn matching + Tarjan SCC) is sound and not dead — 2,530 removals over 27,892 `update` calls — but every removal is recovered by the joint line DP + all-different floor in the same or next pass, so it only reorders when a candidate dies, never whether it dies; node counts are unchanged and no real-app timing was run because the mock verdict already settles it |
...
| Skyscraper 9×9, bitmask DP in one scratch buffer (#134) | timing board (`PUZZLE_LINK_timing.txt`): baseline **45.0 s** / candidate **3.6 s**, ratio **0.08**; shipped board (`PUZZLE_LINK.txt`): 2.5 s / 0.3 s, ratio 0.12 (each a `just time skyscraper --ring-clues` median of 3, 2026-08-27, v2026.08.14-d47fc4b) | — | `prune` now holds each DP layer as a bitmask of running maxima per visible count, in one reused `Uint16Array` instead of `Set`s of `(count, max)` keys rebuilt per call. Deductions are unchanged: the three recovery-probe goldens are byte-identical and a 150,000-state differential against the old `prune` (sizes 4–9) found no difference. Mock: `FUZZ=20000` soundness fuzz 3.5 s → 0.20 s |
| Skyscraper 9×9, exact line DP over digit subsets (#137) | timing board (`PUZZLE_LINK_timing.txt`): baseline **3600 ms** / candidate **0 ms**, ratio **0.00**; shipped board (`PUZZLE_LINK.txt`): 300 ms / 0 ms (each a `just time skyscraper --ring-clues` median of 3, 2026-08-27, v2026.08.14-d47fc4b; per-rep both boards read `first 0ms unique 0ms sum 0ms [unique]` 3/3, the app's readout floor) | — | kept: the DP now keys each layer by the *subset* of sub-peak digits used rather than by position, which fixes the prefix length and the running max at once, and the peak join pairs a left subset with its complement instead of matching visible counts. That makes the sweep exact for a line. Mock nodes: gen_9 762 → **0**, gen_9_timing 6972 → **16**, gen_6 8 → 4, gen_4 0 → 0; probe wall 13.0 s → 0.14 s on gen_9_timing. Per call it costs ~1.8× (`FUZZ=20000` fuzz 0.23 s → 0.41 s, 0 violations both). See `docs/research/137-exact-line-dp.md` |
| Hit counts 9×9 (given-only link, 27 active clues) | >300 s (`[timeout]`, 2026-08-27) | — | the true board exceeds the app time limit, as Skyscraper's did before its joint component (#113) |
...
| ISOFILL 2×2 crossing rule, timed (stripped, 2026-08-27, 3 reps each, recorded app offline, #148) | baseline = the component at `origin/main` (f376514), candidate = with the crossing rule (233b15f). `puzzle-30` **5.2 s → 5.2 s**, ratio 1.00 (reps 5.2/5.2/5.1 vs 5.2/5.2/5.2). `puzzle-32` **3.7 s → 3.9 s**, ratio 1.05 (reps 3.7/3.7/3.7 vs 4.0/3.9/3.9). `puzzle-35-silent` **46.6 s → 47.4 s**, ratio 1.02 (reps 46.6/46.8/46.6 vs 47.4/52.0/47.3). These baselines read a little above the #143 row's candidates (5.2 s against 5.0 s, 46.6 s against 45.6 s) for what is the same component: different session, and only the same-session pair is comparable | — | crossing removed (#148): sound, but no board got faster and two got slower. A probe on `puzzle-32` (a counter in the rule, `app-solve.mjs` relaying `[probe]` lines) fired it 331 times in 46,500 `update` calls — 0.7 % of calls — while every call pays the 81-block scan. The walk rules already refute nearly every checkerboard: a crossing splits a region, and that is what `reach` and `cut` see. The rule and its two directed tests are commit 233b15f, removed in the next commit |
| ISOFILL cut-rule blob gate, timed (stripped, 2026-08-27, 3 reps each, recorded app offline, #150) | baseline = the component at `647fb99`, candidate = with the gate (`2b42a99`). `puzzle-30` **5.2 s → 5.0 s**, ratio 0.96 (reps 5.2/5.2/5.2 vs 5.0/5.0/5.0). `puzzle-32` **3.7 s → 3.7 s**, ratio 1.00 (reps 3.7/3.7/3.7 vs 4.0/3.7/3.7). `puzzle-35-silent` **46.5 s → 45.6 s**, ratio 0.98 (reps 45.8/46.5/46.6 vs 45.6/45.7/45.2). Only `puzzle-30` is an effect rather than spread: both its triples repeat to the app's 100 ms step. `puzzle-32`'s candidate spread 3.7–4.0 s on its own, and `puzzle-35-silent`'s 0.9 s gap sits inside a baseline that spread 0.8 s — read those two as washes | — | gate removed (#150): the cut rule's strand walk runs only when the digit's placed cells sit in more than one blob, which is exact (a path of placed cells is not broken by removing an open cell) and skips **33 %** of strand walks over the harness fuzz set (237,850 of 711,748, from a counter patched into the gate at 2b42a99). A differential test held the deductions identical — gated matched ungated on all 1,500 fuzz states, and an overgated build that never walks differed on 1,461 of them, so the comparison had teeth. But a third of one half of one rule buys 4 % on the one board that moves, nowhere near the 0.9× bar. Measured as its own BFS over the placed cells, not read off the reach walk as ISS reads its door tally (`connected_values.md` §3.1); that costs at most 40 array reads per digit per call against the hundreds-of-cells walk it gates, so the walk-borne form could not turn 0.96 into 0.90. The gate, the differential test and its shared fixtures module are commit 2b42a99, removed in the next commit |
