// Per-call cost, n=9: examples/_shared/HouseGacComponent.js against the #406
// matching filter AllDiffGacComponent.js, both run through their own update()
// on 20000 random consistent states (a hidden permutation plus each other digit
// at 35%). One update call each, no fixpoint loop, 3 reps.
...
for (const [label, component] of Object.entries(components)) {
  ...
  for (let rep = 0; rep < 3; rep++) {
    ...
    console.log(label.padEnd(32), (Number(process.hrtime.bigint() - t0) / states.length / 1000).toFixed(1), 'us/call')
  }
}
