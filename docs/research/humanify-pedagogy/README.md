...
| The reference as a page | `node docs/research/humanify-pedagogy/tools/build-ref.mjs docs/research/bundle-api-reference.md <out.html> --public-only` (needs `npm ci` for `marked`), then republish the artifact at `https://claude.ai/code/artifact/6f166095-f641-4edc-8485-69cb61380118` with the Artifact tool, passing that `url`. **The shared artifact is always the `--public-only` build**: the app's author asked that the solver internals not be shared, so that build keeps only entries whose Access line is **public**, drops "Under the hood", the reachable tier and the tier buttons, and cuts the guide passages fenced `<!--private-->…<!--/private-->` in `tools/guide.md`. Without the flag you get the full page for local reading; never publish it. The page is a guide (`tools/guide.md`) in front of the reference, reordered and regrouped by `tools/page.json`; the builder strips bundle line citations, renders mermaid fences as `<pre class="mermaid">` (the artifact host renders them), and fails if a guide or task link names an entry that does not exist. The earlier page at `…/f1c77297-9319-45a5-9551-789c90b7b4c5` is superseded. |
