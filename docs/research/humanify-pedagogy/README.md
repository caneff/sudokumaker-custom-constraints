...
| The reference as a page | `node docs/research/humanify-pedagogy/tools/build-ref.mjs docs/research/bundle-api-reference.md <out.html> --public-only` (needs `npm ci` for `marked`), then republish the artifact at `https://claude.ai/code/artifact/6f166095-f641-4edc-8485-69cb61380118` with the Artifact tool, passing that `url`. **The shared artifact is always the `--public-only` build**: the app's author asked that the solver internals not be shared, so that build keeps only entries whose Access line is **public**, drops "Under the hood", the reachable tier and the tier buttons, cuts the guide passages fenced `<!--private-->…<!--/private-->` in `tools/guide.md`, and scrubs passing mentions: any sentence, list item or table row that names a non-public class or method (`SolverState`, `puzzle.state`, `extends CompositeComponent`, …) is dropped, `solverState` parameters read `state`, and the build fails if such a name survives. Without the flag you get the full page for local reading; never publish it. The page is a guide (`tools/guide.md`) in front of the reference, reordered and regrouped by `tools/page.json`; the builder strips bundle line citations, renders mermaid fences as `<pre class="mermaid">` (the artifact host renders them), and fails if a guide or task link names an entry that does not exist. The earlier page at `…/f1c77297-9319-45a5-9551-789c90b7b4c5` is superseded. |
...
- The page is shared outside this repo: no pointers into other repo docs, no
  notes about the document's own history. Only the `--public-only` build is
  shared (see the table above); a guide sentence that names a non-public
  entry goes inside `<!--private-->…<!--/private-->` or the public build fails
  its link check.
...
| `tools/anchor-probe.mjs` | shot-scraper probe for the built reference page: clicks three far-apart in-body links and asserts each target lands within 0..20px of the viewport top, catching a hash jump that stops short. |
