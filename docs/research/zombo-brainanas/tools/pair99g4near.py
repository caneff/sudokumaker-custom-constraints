...
from that grid and the earlier ones of this run. usage: pair99g4near.py 0 1 [nseeds] [limit] [stall] -> hunt/pair99g4near/full_UI_w3_s<seed>.json"""
...
OUT = Z + "hunt/pair99g4near/"; os.makedirs(OUT, exist_ok=True)
...
for i, (kind, sh) in enumerate(kinds):
    ...
    avoid = [{p: int(json.load(open(Z + "hunt/pair99g4/full_UI_w3_s0.json"))["infected"][p[0]][p[1]] == "*") for p in zb.CELLS}]
