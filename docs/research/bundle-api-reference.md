...
- There are two `helpers` objects, one for setup code and one for component
  functions; the table under "Shared reads" says which members each has.
...
- Access. Each class section opens with an **Access** line. **public**: the
  app injects it into custom code by name, or hands it to you through
  `puzzle` or `helpers`; these are the things a custom constraint is meant to
  use. **reachable, not documented**: you can get an instance (for example
  `puzzle.state`, or a graph yielded by a helper) but no documented surface
  names it, so its shape may change without notice. **internal**: nothing in
  custom code can reach it; described so the solver's behaviour can be
  understood, not for calling.
- Confidence. Every entry was written from the function body unless it
  ...
  Entries in the internals sections also carry where the body was
  read in full.
...
Forwards straight to `SolverState.setRegions`, creating the region components.
...
The base object comes from `createHelpers(spec)` (`bundle.claude.js:1614`); the
extended one from `createExtendedHelpers(spec, sudokuState)`
(`bundle.claude.js:9201`), which spreads the base and adds `lines`, `misc` and a
region-aware `geometry`. Main code runs under `setupPuzzle`, which builds the
extended object (`bundle.claude.js:9349`); a custom component class is compiled
with a bare `createHelpers(puzzleSpec)` (`bundle.claude.js:9994`).
...
#### `getX(cellId)` / `getY(cellId)`
The 0-based column and row of a cell, the same arithmetic as
`helpers.cellIds.getX` and `getY` (no range check).
...
#### `static create()`
Returns a fresh empty `SolverState` — every cell valueless with all digits as candidates, no regions, no components.
...
- **Notes:** names the regions `box N` when `regionsAreRectangularBoxes` says the layout is rectangular boxes, else `region N`. Reached from setup code as `puzzle.setRegions`.
...
- **Notes:** called from `addConstraintComponent`, not directly.

#### `getConstraintComponents()`
Returns the live `Set` of every registered component.

#### `getHouseComponents()`
Returns the live `Set` of registered `HouseComponent`s only.

#### `addComponentListener(listener)`
Subscribes `listener` to the `"change"` event, called with `{ type: "add" | "delete", component }` on every registration and unregistration. Used by `ComponentSubscription.bind`.
...
- **Notes:** short-circuits as soon as the running intersection empties. This is what a naked-subset style elimination calls to find its targets; reached from a component as `puzzle.getCellsSeenByCells`.
...
- **Mutates:** the cell's `candidates`, `candidateSetMap`, `updateSet`.
...
- **Notes:** a digit already placed enough times (`placedCount >= repeatCount + 1`) or with no candidate cells left records no set.
...
- **Notes:** this is the call a component makes to tell the solver its cells must contain certain digits; `addConstraintComponent` uses it to require all digits in every house.

#### `getSetsForCandidate(digit)`
Returns that digit's required-digit sets from the candidate-set map, shortest first. Read by hidden-single and pointing logic steps.

#### `getCloneSet(cellId)`
Returns the `Set` of cell ids that always hold the same digit as `cellId`, including itself. Non-empty even with no clone constraints.
...
#### `isSolved()`
True when every cell has a value. Says nothing about validity — `findSolutions` pairs it with `validate()`.

#### `clone()`
Returns `new SolverState(this)`: per-cell value and candidates copied, `constraintComponentByCell` and `candidateSetMap` structurally copied, `clonedCellsMap` deep-copied, and the component set copied **by reference**, so every search node shares one component object.

#### `isTerminalChange(change)`
True for `ReplaceComponent` and `AbortSolver` — the two change types after which the solver stops draining a component's generator.
...
## Coordinates, ids and connectivity
...
`helpers.connectivity`, `helpers.lines` and `helpers.misc`. `lines` and `misc`
are on the setup-time `helpers` only (the table under "Shared reads" above);
built-in components, which the handlers build during setup, use both freely.
...
Converts a cell id to its 0-based column and row via `getX`/`getY`.
...
#### `getAllCellIds()`
Lists every cell id on the board.
...
Converts a corner id to grid-point coordinates on the (width+1) by (height+1) lattice of cell corners.
...
Converts an outer-cell id to its coordinates in the ring around the board.
...
Gives the centre point of an outer-cell slot.
...
The undirected graph at `bundle.claude.js:250`. `helpers.connectivity.getOrthogonallyConnectedGroups` lists the members you will use most (`getPoints`, `getEdges`, `hasCycles`, `isSimpleLines`, `toArrays`); this entry covers the rest of the class.
...
- **Notes:** it iterates the point's live neighbour set while `removeEdge`
  ...
  removed disappears mid-loop.

#### `hasPoint(point)`
Whether the interned point is present. **Returns:** boolean.
...
- **Notes:** this is what `helpers.connectivity.getOrthogonallyConnectedGroups`
  hands you — a generator, so spread it if you need two passes.
...
- **Notes:** each yielded array is the *full* connected set reachable from a
  seed, so it can include points that were not in your input.
...
- **Notes:** a `startPoint` not in the graph yields an empty set, since the
  loop skips points with no entry.
...
- **Returns:** a new `LineGraph` with the same comparator, holding those points
  ...
  so isolated ones survive (unlike the `DirectedLineGraph` twin).
...
- **Notes:** edges only — a point with no edges is lost by the clone, and so
  by `toArrays()`, which clones first.
...
Picks the first and last cell of a line.
...
Drops a line's two end cells and keeps the interior.
...
#### `intersect(otherSet)`
Keeps only the digits present in both sets, by ANDing the masks in place.
- **Returns:** `this`. **Mutates:** the receiver.
...
Copies the left set and adds every element of the right one (`unionOfSets`, `bundle.claude.js:99`).
...
generators that `yield` change objects; the change-type names below
(FilterCandidatesAtCell, RemoveCandidatesFromCells, AbortSolver, …) are the
app's own enum names, mapped to the factory functions in the "Change objects"
table above. And a built-in `validate` returns `ValidResult = {valid: true}`
(`bundle.claude.js:2675`) or `{valid: false, message}` — not a boolean.
...
### NegativeBetween (`class NegativeBetweenComponent extends ConstraintComponent`)
...
### NegativeDifference (`class NegativeDifferenceComponent extends PairComponent`)

Access: **public** — registered built-in; construct it as `new <Name>Component(...)` and hand it to `puzzle.addConstraintComponent` or `replaceComponent`.

Two cells whose difference is none of the listed values. `bundle.claude.js:6238`, extends PairComponent.
...
### NegativeIndex (`class NegativeIndexComponent extends ConstraintComponent`)
...
### NegativeRatio (`class NegativeRatioComponent extends PairComponent`)

Access: **public** — registered built-in; construct it as `new <Name>Component(...)` and hand it to `puzzle.addConstraintComponent` or `replaceComponent`.

Two cells whose ratio is none of the listed values. `bundle.claude.js:6365`, extends PairComponent.
...
### NegativeSum (`class NegativeSumComponent extends CompositeComponent`)

Access: **public** — registered built-in; construct it as `new <Name>Component(...)` and hand it to `puzzle.addConstraintComponent` or `replaceComponent`.

Cells whose total is none of the listed sums. `bundle.claude.js:6472`, a CompositeComponent.
...
### PredefinedCandidates (`class PredefinedCandidatesComponent extends ConstraintComponent`)
...
### Product (`class ProductComponent extends CompositeComponent`)

Access: **public** — registered built-in; construct it as `new <Name>Component(...)` and hand it to `puzzle.addConstraintComponent` or `replaceComponent`.

Cells whose digits multiply to one of the listed products. `bundle.claude.js:6574`, a CompositeComponent.
...
### RatioComponent (`class RatioComponent extends PairComponent`)

Access: **public** — registered built-in; construct it as `new <Name>Component(...)` and hand it to `puzzle.addConstraintComponent` or `replaceComponent`.

Two cells in one of the listed `1 : r` ratios. `bundle.claude.js:4009`, extends PairComponent.
...
### RequiredDigits (`class RequiredDigitsComponent extends ConstraintComponent`)
...
### RequiredGroups (`class RequiredGroupsComponent extends ConstraintComponent`)
...
### SameDigit (`class SameDigitComponent extends ConstraintComponent`)
...
### SameGroup (`class SameGroupComponent extends ConstraintComponent`)
...
### SameSum (`class SameSumComponent extends ConstraintComponent`)
...
### SandwichSum (`class SandwichSumComponent extends CompositeComponent`)

Access: **public** — registered built-in; construct it as `new <Name>Component(...)` and hand it to `puzzle.addConstraintComponent` or `replaceComponent`.

The digits between the two crust digits on a line sum to `sum`. `bundle.claude.js:7525`, a CompositeComponent.
...
### SelfCounting (`class SelfCountingComponent extends CompositeComponent`)
...
### Sequence (`class SequenceComponent extends CompositeComponent`)

Access: **public** — registered built-in; construct it as `new <Name>Component(...)` and hand it to `puzzle.addConstraintComponent` or `replaceComponent`.

Cells whose digits, in order, form an arithmetic progression with one constant step. `bundle.claude.js:7853`, a CompositeComponent.
...
### Skyscraper (`class SkyscraperComponent extends ConstraintComponent`)
...
### Sum (`class SumComponent extends CompositeComponent`)
...
### WeakLink (`class WeakLinkComponent extends ConstraintComponent`)
...
### WeakLinks (`class WeakLinksComponent extends ConstraintComponent`)
...
### WeightedSum (`class WeightedSumComponent extends ConstraintComponent`)
...
### XSum (`class XSumComponent extends CompositeComponent`)

Access: **public** — registered built-in; construct it as `new <Name>Component(...)` and hand it to `puzzle.addConstraintComponent` or `replaceComponent`.

The digit in `xCell` says how many cells of the line, counted from its start, sum to `sum`. `bundle.claude.js:8321`, a CompositeComponent.
...
- **Notes:** the memo key comes from `getFriendCacheKeyBuilder(paramType)` and
  ...
  Digits below `minDigit` are holes in the returned array.
...
- **Notes:** no-op when `knownDigit` is not yet resolved to a single cell.
  Distance is measured in positions along `cellIds`, not geometrically.
...
- **Notes:** step sizes run from `minimumDifference` up to
  ...
  progression is kept only if every cell still holds its digit.
...
#### `getMinSum(state, cellWeights, excludedCellId)` / `getMaxSum(state, cellWeights, excludedCellId)`

The weighted twins of `SumCandidateUpdater.getMinSum` / `getMaxSum`: each cell's
value (or its smallest / largest candidate) is multiplied by its weight. The
only other difference is the argument: a `Map` of cell id to weight, not an
array of ids.
...
- **Mutates:** the `SolverState` it holds.

#### `*processChange(change)`
Dispatches one change object to the matching applier method by `change.type`.
- **Returns:** generator of deduction results.
- **Notes:** `AbortSolver` yields `{ type: "failed", cells: change.cells || [], message: change.message }` directly. A type outside `ChangeType` yields nothing.

#### `*setValueAtCell(value, cell)`
Sets `cell` to `value` through `SolverState.setValueAtCell`, yielding its one result.

#### `*filterCandidatesAtCell(digitMask, cell)` / `*filterCandidatesAtCells(digitMask, cells)`
Intersects the candidates of one cell, or of each listed cell, with `digitMask` via the matching `SolverState` method, yielding one result per cell.

#### `*removeCandidatesFromCell(digitMask, cell)` / `*removeCandidatesFromCells(digitMask, cells)`
Clears the bits of `digitMask` from one cell's candidates, or from each listed cell's, yielding one result per cell.
...
- **Notes:** the recording is also gated on the module-level `verboseSolvingEnabled`, so it stays silent if the flag was flipped after construction.
...
- **Notes:** this is what `Solver.getDeductionResultsForTransport` returns.

#### `processChange(change)`
Dispatches one change by type, exactly as in `ChangeApplier`, but returns a generator of `[affectedCellId, result]` pairs; an unknown type returns `[]`.

#### `*setValueAtCell(value, cell)`
Sets the cell's value as `ChangeApplier` does, but yields `[cell, result]` only when the result is not `unchanged`, after passing it through `ensureErrorMessage`.
...
- **Notes:** the plural form loops the single-cell state method rather than calling the state's plural generator as `ChangeApplier` does. Same effect.
...
- **Notes:** same loop-versus-plural-generator difference from `ChangeApplier`, same effect.

#### `reportBroken(cells, message)`
Yields one `[undefined, { type: "failed", cells, message }]` pair for an `AbortSolver` change; the `undefined` cell id is what keeps the abort out of the affected-cell highlighting.

#### `ensureErrorMessage(result)`
Fills in a missing message on a `failed` result with `describeCellsWithNoCandidates(result.cells)`, and returns the result.
...
#### `getBestCell()`
Scans all cells and returns the unsolved one with the lowest weight, or `undefined` if the grid is full.

#### `getSortedCells()`
Returns every unsolved cell sorted by weight, cheapest first. Not called from `findSolutions`; it exists for callers that want the whole ordering.

#### `getBetterCell(cellA, cellB)`
Returns whichever of the two cells has the lower weight, preferring `cellA` on a tie.

#### `compareCells(cellA, cellB)`
A `-1 / 0 / 1` comparator on the two cells' weights.
...
- **Notes:** so fewer candidates dominates, more constraints breaks ties toward the more constrained cell, and the random term makes the search non-deterministic. Capping at 3 means a 4-candidate cell and a 9-candidate cell rank alike. Memoized per instance, so it is stale-free only because a new ranking is built at each branch point.
...
#### `initialize(solverState)`
Binds to the state, then seeds `components` from the current component set, calling `onAdd` for each match.
...
- **Notes:** a `"delete"` event deletes unconditionally and calls `onDelete`, even for components the filter never accepted.

#### `getComponents()`
Returns the live `Set` of matching components, not a copy.

#### `getState()`
Returns the accumulator object handed to `onAdd` / `onDelete`.
...
- **Notes:** this is how a logic step survives `SolverState.clone` at a branch point — the logic step's constructor takes the previous step and clones its cache.
...
Adds `handler` to that event's set, creating the set if needed.

#### `off(eventName, handler)`
Removes one handler, or clears every handler for the event when `handler` is omitted.

#### `emit(eventName, payload)`
Calls each handler with `payload`, over a copied array so a handler may subscribe or unsubscribe during dispatch.
...
#### `clone()`
Returns a new `CloneableMap` with the same keys and a fresh `Set` per value, so the clone's per-cell sets are independent while the component objects inside stay shared by reference.
...
#### `addSet(setInfo)`
Appends a set-info to its digit's list and re-sorts that list.

#### `removeCandidate(digit, cellId)`
Drops `cellId` from every set recorded for `digit`, deletes any set left empty, then re-sorts.
- **Notes:** called by `SolverState.filterCandidatesAtCell` and `removeCandidatesFromCell` for each digit actually eliminated. A set shrinking to one cell is the hidden single.

#### `reduceCandidateSetsAt(digit, cellId)`
Records that `digit` has been placed at `cellId`: removes every set containing that cell, except that a set with a nonzero `repeatCount` is kept and its count decremented instead.

#### `getSets(digit)`
Returns the live array of set-infos for that digit, shortest first.

#### `sortSets(sets)`
Sorts a set-info array ascending by `cells.length`, so the most constrained set is first.
...
Replaces the solver's logic-step list. Called by `StandardLogicStepsGenerator` or `CustomLogicStepsGenerator` right after the solver is built.
...
- **Notes:** returns silently if the initial `updateConstraintsAndValidate` fails, an invalid step is reached, or a branch value is immediately contradictory. Clones the state per branch digit, so nothing is undone by backtracking — it is abandoned.

#### `singleLogicStep()`
Takes exactly one logic step; a bare alias for `step()`, and the entry point used by the worker's single-step message.

#### `getDeductionProcessor(deduction)`
Returns a `VerboseChangeApplier` when verbose solving is enabled, otherwise a `ChangeApplier`, wrapping this solver's state and the given deduction request.
...
- **Notes:** `deductionResults` is present only in verbose mode; `findSolutions` treats its presence as "logic made progress, do not branch yet".

#### `getDeductionResultsForTransport(applier)`
Returns `applier.getVerboseResults()` for a `VerboseChangeApplier`, and `[]` for a plain one.

#### `log(message)`
Prints `message` to the console indented by two spaces per search depth.
...
Stores `handler` under `constraintType`, defaulting `handler.priority` to `0`. A second registration for the same type replaces the first.
...
- **Notes:** the two passes are why a handler can look at components other handlers created (anti-king checks `getCellsSeeEachOther` before adding its own). All handlers share one `puzzle` view and one `helpers` object.
...
- **Notes:** everything below reads grid size and digit range from this module global, so it must run before any state is created.

#### `disableVerboseSolving()`
`bundle.claude.js:1657`. Flips `verboseSolvingEnabled` to `false` for the rest of the worker's life, so `Solver.getDeductionProcessor` builds plain `ChangeApplier`s and no step explanations are produced. One-way; nothing turns it back on.
...
- **Returns:** the buffer, which the worker transfers (not copies) to the main thread.
...
- **Notes:** drops `repeatCount`, which `CandidateSetMap.addSet` stores and `reduceCandidateSetsAt` reads — so a cloned state's sets look repeat-free. Flagged as observed, not as an intended behaviour.

#### `registerConstraintHandler(constraintType, handler)`
`bundle.claude.js:9392`. Registers a handler on the singleton registry; the bundle calls it once per built-in constraint type at module load.

#### `getNextConstraintId()`
`bundle.claude.js:9486`. Returns and increments a module-level counter, giving each constraint a unique id within the session.
...
`bundle.claude.js:9763`. Wraps `createDefaultConstraintConfig` into a full constraint record `{ id, name: undefined, enabled: true, solverIgnored: false, config }`, defaulting `id` from `getNextConstraintId()`.

#### `getSolverEnvironment()`
`bundle.claude.js:9908`. Returns `{ verboseSolving: verboseSolvingEnabled }` — the one global flag exposed to constraint code, so a component can skip building step explanations it knows will be discarded.
...
- **Notes:** the `false` third argument to `setValueAtCell` is what keeps givens from being treated as solver deductions. This is the caller that makes `initialize` run once per component, after givens and before the first full update.

#### `handleStepMessage()`
`bundle.claude.js:11539`. Worker handler for a single-step request: calls `activeSolver.singleLogicStep()`, attaches a serialized grid, and posts `{ type: "update", ... }` with the buffer transferred. Posts `{ type: "error" }` if no solver has been started.

#### `handleFindNextMessage()`
`bundle.claude.js:11550`. Worker handler for "next solution": lazily creates the `findSolutions` iterator, pulls one solution, and posts it with `changed` set by comparing against the initial grid snapshot. When the iterator is exhausted it posts `changed: false` and a `valid` computed from `isSolved()` and `validate()`.
...
- **Notes:** synchronous and unbounded — it blocks the worker until the search finishes, which is why a multi-solution puzzle can hang the tab.
...
Every step's `clone(state)` is the same shape and is not listed per step: a new
step over the cloned state carrying only the step's configuration (`groupSize`,
`useRandomness`). The steps that keep a `ComponentSubscription` or a cache pass
`this` as the source, so the subscription is cloned rather than rebuilt and the
cache is deep-copied per search node; those cache classes keep their own
`clone()` entry below.
...
`bundle.claude.js:1985` and `:1988`.
...
remove produce no clause. `bundle.claude.js:2264`.
...
- **Cost:** a full state clone plus a constraint fixpoint per candidate tried —
  cells × candidates clones in the worst case.
...
and returns the `"fail"` sentinel on the first failure, else `null`.
...
- **Cost:** two `iterateCombinationsOfSize(sets, groupSize)` enumerations per
  digit, so it grows sharply with `groupSize`.
...
- **Notes:** because it reads candidate *sets* rather than houses, it also
  fires on the required-digit sets a constraint registered.
...
#### `*execute()`
Runs `check` on every house component.
...
- **Cost:** `C(prefiltered cells, groupSize)` over the whole grid, not per
  ...
  prefilter usually keeps the pool small.
...
- **Notes:** the deduction is yielded unconditionally, even when no digit is
  removable, so the applier reports `unchanged` in that case.
...
order: max-count, house-exclusion, digit-limitations.
...
#### `*execute()`
Runs `executeForComponent` over every tracked kropki component.
...
- **Notes:** skipped for a ratio component whose combined mask includes digit 0,
  where the middle-digit argument does not hold.
...
#### `*execute()`
Runs `pointDigits`, then `reduceCombinationsByContainment`.
...
- **Mutates:** the cached `info.combinations`.
...
- **Cost:** one pass over the component's cells per repeated digit.
...
Same for columns, via `helpers.geometry.getCellsInColumn`.
...
- **Notes:** the loop is over `sudoku.cells` without an explicit solved-cell
  skip; a solved cell simply fails the two-candidate test.
...
#### `setLogicSteps(solver)`
Delegates straight to the wrapped standard generator.
...
- **Returns:** `this`. **Mutates:** the receiver. A line of one point adds
  nothing at all, since there is no lone-point storage.
...
#### `hasPoint(point)`
Whether the interned point is in the graph. **Returns:** boolean.

#### `getPoints()`
Every point, in insertion order.
- **Returns:** a new array; mutating it does not touch the graph.
...
- **Notes:** this does **not** intern its argument, unlike `hasEdge` and
  ...
  and you get the empty set.

#### `getPointsAfter(point)`
The successor set, with the same live-set and no-interning caveats.
- **Returns:** the graph's own `Set`, or a fresh empty `Set`.

#### `getPointCount()`
Size of `points`. **Returns:** number.

#### `isEmpty()`
Whether the graph holds no points. **Returns:** boolean.
...
- **Returns:** generator of `[fromPoint, toPoint]` pairs. **Mutates:** nothing.
  ...
  makes each edge appear once.

#### `hasMergePoints()`
Whether any point has more than one predecessor.
- **Returns:** boolean. This is the "two lines join here" test.
...
- **Notes:** recursive, so a pathological graph could overflow the stack; at
  thermometer scale that never happens.
...
- **Notes:** the backward walk runs first and marks points visited, so the
  ...
  connected set, not a directed reachable set.
...
- **Notes:** the start-point picker is written `!before || before`, which is
  ...
  `Error("This should never happen")` after 1000 total steps.

#### `clone()`
Rebuilds a graph from `getEdges()`.
- **Returns:** a new `DirectedLineGraph`. **Mutates:** nothing. Points are
  shared, not deep-copied, which is safe because they are interned.
...
#### `get magnitude`
`Math.hypot(x, y)`. **Returns:** number. **Mutates:** nothing.

#### `get magnitudeSqr`
`x*x + y*y`, avoiding the square root when you only compare lengths.
**Returns:** number. **Mutates:** nothing.
...
- **Returns:** `this`. **Mutates:** the receiver, not the argument.

#### `Vector2.from(source)`
Builds a `Vector2` from anything with `x` and `y`.
- **Returns:** a new `Vector2`. **Mutates:** nothing.

### Top-level functions (sets, iteration, arrays, numbers, vectors, cloning)

Access: **internal** — not reachable from custom code.

Most of the free functions here are the bodies behind a `*Utils` namespace
member documented in the digit-sets section; the member's entry describes the
behaviour, and the tables below only map the internal name to the public one and
add what the member entry leaves out. The functions with no public wrapper keep a
full entry.

**Sets** — behind `SetUtils`. An `options.comparator` path, where present,
decides membership by calling `comparator(a, b)` pairwise, at O(n·m).

| Function | Behind | Where the wrapper entry does not say it |
|-|-|-|
| `takeOneFromSet(sourceSet)` (`bundle.claude.js:46`) | `SetUtils.takeOne` |  |
| `deleteAllFromSet(targetSet, itemsToDelete, options)` (`bundle.claude.js:50`) | `SetUtils.deleteAll` | comparator called `(itemToDelete, existingItem)` |
| `differenceOfSets(sourceSet, itemsToRemove, options)` (`bundle.claude.js:60`) | `SetUtils.difference` | comparator called `(existingItem, itemToRemove)` — the reverse of `deleteAll` |
| `symmetricDifferenceOfCollections(leftCollection, rightCollection)` (`bundle.claude.js:70`) | `SetUtils.symmetricDifference` | dispatches to the fast path below when both sides have `has` |
| `symmetricDifferenceOfSets(leftSet, rightSet)` (`bundle.claude.js:78`) | `SetUtils.symmetricDifference` | the set-only fast path, two `has` scans; no comparator |
| `retainIntersectionInSet(targetSet, otherSet)` (`bundle.claude.js:86`) | `SetUtils.filter` | takes a set, never a predicate, despite the name |
| `intersectionOfSets(sourceSet, otherSet)` (`bundle.claude.js:90`) | `SetUtils.intersection` |  |
| `addAllToSet(targetSet, itemsToAdd)` (`bundle.claude.js:95`) | `SetUtils.addAll` |  |
| `unionOfSets(leftSet, rightSet)` (`bundle.claude.js:99`) | `SetUtils.union` |  |
| `setsAreEqual(leftSet, rightSet, options)` (`bundle.claude.js:104`) | `SetUtils.isEqual` | `size` gate first, even on the comparator path |
| `setHasAllOf(containerSet, items, options)` (`bundle.claude.js:109`) | `SetUtils.hasAll` | comparator called `(candidateFromSet, item)`; true for empty `items` |
| `setHasSomeOf(containerSet, items, options)` (`bundle.claude.js:126`) | `SetUtils.hasSome` | false for empty `items` |
| `setHasSomeWhere(sourceSet, predicate)` (`bundle.claude.js:137`) | `SetUtils.hasSomeWhere` |  |

**Iteration** — behind `IterationUtils`.

| Function | Behind | Where the wrapper entry does not say it |
|-|-|-|
| `getFirstOfIterable(iterable)` (`bundle.claude.js:155`) | `IterationUtils.getOne` | advances a generator by one step |
| `iterateRangeExclusive(startValue, endValue)` (`bundle.claude.js:158`) | `IterationUtils.getRange` |  |
| `iterateRangeInclusive(startValue, endValue)` (`bundle.claude.js:161`) | `IterationUtils.getRangeInclusive` |  |
| `iterateCombinationsOfSize(items, size)` (`bundle.claude.js:164`) | `IterationUtils.getCombinations` | the shared index array is mapped, not yielded |
| `countOccurrencesByValue(items)` (`bundle.claude.js:183`) | `IterationUtils.getCounts` |  |
| `getBestByScore(items, scoreFn, fallback)` (`bundle.claude.js:188`) | `IterationUtils.getBest` |  |
...
object throw on `Object.entries`.
...
shift semantics and silently sets the wrong bit.

#### `listDigitsInMask(mask)` — `bundle.claude.js:517`
Aliased as `digitsInMask`. Pops the lowest set bit repeatedly.
**Returns:** a new array of digits, ascending. **Mutates:** nothing.
...
compare with `!!` if you care. **Mutates:** nothing. Mask 0 gives `0`.
...
**Numbers** — behind `MathUtils`.

| Function | Behind | Where the wrapper entry does not say it |
|-|-|-|
| `sumOfNumbers(numbers)` (`bundle.claude.js:704`) | `MathUtils.sum` | accumulates from 0 |
| `productOfNumbers(numbers)` (`bundle.claude.js:709`) | `MathUtils.product` | accumulates from 1 |
| `modulo(dividend, divisor)` (`bundle.claude.js:714`) | `MathUtils.mod` | `((a % b) + b) % b`; also drives `ArrayUtils.sliceWrapped` |
| `getPrimeFactors(number)` (`bundle.claude.js:740`) | `MathUtils.getFactors` | tabled primes to 83, then odd candidates checked with `isPrime`; 0 never terminates |
| `radiansToDegrees(radians)` (`bundle.claude.js:761`) | `MathUtils.toDegrees` |  |
| `degreesToRadians(degrees)` (`bundle.claude.js:764`) | `MathUtils.toRadians` |  |

**Vectors** — behind `Vector2Funcs`, all over plain `{x, y}`. Every one
returns a fresh object or a scalar and mutates no argument.

| Function | Behind | Where the wrapper entry does not say it |
|-|-|-|
| `getVectorMagnitude(vector)` (`bundle.claude.js:826`) | `Vector2Funcs.getMagnitude` | `Math.hypot` |
| `getNormalizedVector(vector)` (`bundle.claude.js:829`) | `Vector2Funcs.normalized` | zero vector gives `NaN`s |
| `getScaledVector(vector, scaleFactor)` (`bundle.claude.js:833`) | `Vector2Funcs.scaled` |  |
| `addVectors(vectorA, vectorB)` (`bundle.claude.js:836`) | `Vector2Funcs.sum` | name clash with `MathUtils.sum` |
| `subtractVectors(vectorA, vectorB)` (`bundle.claude.js:839`) | `Vector2Funcs.difference` | `a - b` |
| `addScaledVector(vectorA, vectorB, scaleFactor)` (`bundle.claude.js:842`) | `Vector2Funcs.scaledSum` | `a + b * scaleFactor` |
| `getDistanceBetweenPoints(pointA, pointB)` (`bundle.claude.js:848`) | `Vector2Funcs.getDistance` |  |
| `getAngleBetweenVectors(vectorA, vectorB)` (`bundle.claude.js:854`) | `Vector2Funcs.getAngle` | `acos` of the dot of two normalized temporary copies; rounding can give `NaN` near parallel |
| `getManhattanDistanceBetweenPoints(pointA, pointB)` (`bundle.claude.js:862`) | `Vector2Funcs.getManhattanDistance` |  |
| `getRotatedVector(vector, angle)` (`bundle.claude.js:865`) | `Vector2Funcs.getRotated` |  |
| `getClampedVector(point, rect)` (`bundle.claude.js:873`) | `Vector2Funcs.getClamped` | clamps into `{x, y, width, height}` |
| `compareVectorsInReadingOrder(pointA, pointB)` (`bundle.claude.js:879`) | `Vector2Funcs.compareVectors` | `y` first, then `Math.sign` of the `x` difference |
| `isVectorAfterInReadingOrder(pointA, pointB)` (`bundle.claude.js:886`) | `Vector2Funcs.isVectorGreaterThan` |  |
| `getAverageVector(points)` (`bundle.claude.js:889`) | `Vector2Funcs.getAverage` | empty input gives `{x: NaN, y: NaN}` |
...
- **Notes:** no Oxford comma, and it indexes `values.length`, so it needs an
  ...
  input returns `""`. Used for constraint names in solver explanations.
...
- **Notes:** this is what makes `SumsHelper.getCombinationsForSumWithoutRepeat`
  ...
  results correctly, since it tests with `cache.has`.


**Arrays** — behind `ArrayUtils`. The copy/in-place twins are easy to confuse:
`arrayWithoutAll` copies, `removeValuesInPlace` does not.

| Function | Behind | Where the wrapper entry does not say it |
|-|-|-|
| `removeFirstValue(array, value, options)` (`bundle.claude.js:1676`) | `ArrayUtils.removeFirst` | comparator called `(value, candidate)` |
| `arrayWithoutAll(array, valuesToRemove, options)` (`bundle.claude.js:1688`) | `ArrayUtils.withoutAll` | copies; `Set` fast path, comparator path dedups the removal list |
| `removeValuesInPlace(array, valuesToRemove, options)` (`bundle.claude.js:1702`) | `ArrayUtils.remove` | in place: empties the array and pushes the keepers back |
| `removeWhereInPlace(array, predicate)` (`bundle.claude.js:1719`) | `ArrayUtils.removeWhere` | same empty-and-refill |
| `arrayIncludesSome(array, values, options)` (`bundle.claude.js:1725`) | `ArrayUtils.includesSome` |  |
| `arrayIncludesEvery(array, values, options)` (`bundle.claude.js:1735`) | `ArrayUtils.includesEvery` |  |
| `shuffledCopy(array)` (`bundle.claude.js:1745`) | `ArrayUtils.shuffled` | Fisher-Yates over a `slice()` copy, `Math.random` |
| `mapIterableToArray(iterable, mapFn)` (`bundle.claude.js:1753`) | `ArrayUtils.mapIterable` | `mapFn` gets the element only, no index |
| `chunkArray(array, chunkSize)` (`bundle.claude.js:1765`) | `ArrayUtils.chunk` | `chunkSize <= 0` gives one chunk holding everything |
| `arraysAreSameLength(...arrays)` (`bundle.claude.js:1774`) | `ArrayUtils.areSameLength` |  |
| `hasDuplicatesByComparator(array, comparator)` (`bundle.claude.js:1789`) | `ArrayUtils.hasDuplicates` | the O(n²) comparator path only; the wrapper uses a `Set` without one |
| `withoutDuplicatesByComparator(array, comparator)` (`bundle.claude.js:1800`) | `ArrayUtils.withoutDuplicates` | the O(n²) comparator path; membership by `Array.prototype.find`, so a stored `undefined` reads as absent |
| `toArrayMaybeUnique(value, { unique = false })` (`bundle.claude.js:1814`) | `(internal to the comparator paths)` | returns the *same* array when `unique` is false and the input is an array |
...
- **Notes:** this is how the Thermometer handler turns branching or merging
  ...
  is fine for thermometers and would not be for a dense graph.
...
- **Notes:** the `Number` form assumes the parameter is under 1000; larger values
  would collide with the digit-range part of the key.
...
- **Notes:** this is the cache entry shape the sum logic step stores per
  component; `required` starts empty and is narrowed later.
...
the shared sum-combination cache can handle.
...
- **Notes:** cells with region id `-1` (unassigned) are dropped, and region
  indices with no cells come back as empty arrays rather than holes.
...
region per row for a square grid of side at most 9, else all `-1`.

#### `getStandardBoxLayout(size)`

`bundle.claude.js:8542`. Returns the rectangular box layout for a 4x4, 6x6, 8x8
or 9x9 grid, or `undefined` for any other size.
...
- **Notes:** bails immediately unless the cell count is a multiple of the digit
  ...
  `ceil(sqrt(digitCount))`, so the squarest box wins.
...
- **Notes:** assumes the grid divides evenly by the box; it does no checking, so
  a bad `boxSize` yields fractional region ids.
...
- **Notes:** each window must be split horizontally, split vertically, or have
  ...
  necessary local test, not a proof of a global rectangular tiling.
...
- **Notes:** in the bundle both masks are built with the same predicate
  ...
  A low/high pair still passes; so would a pair of low plus anything.
...
`"123 456 789"` or `0_5` with `"01 23 45"`.
...
- **Notes:** with a multi-cell bulb, single-cell arrows are folded into the
  ...
  two-cell bulb reads as a two-digit number.
...
- **Notes:** custom puzzles short-circuit to `""` with no checking. It counts
  region sizes rather than verifying shape, so disconnected regions pass.
...
indexes its row with its 1-based column number.
...
is the forced triangular number of `maxDigit`.
...
opposite fortress kind, and `""` otherwise.
...
- **Notes:** a digit range with no table entry passes everything.
