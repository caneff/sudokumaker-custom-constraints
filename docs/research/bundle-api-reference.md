...
Each is also flagged in place as a `Discrepancy` note. Status as of
2026-09-13, after working the list against the primary docs on `main`:

| # | Finding | Status |
|-|-|-|
| 1 | `gotchas.md` §2 called a validate-only component inert. `SolverState.validate` (9045) runs every component whose `validateDuringSolve` is true, and the wrapper sets it whenever a segment defines `validate` (10050). | **Resolved by live probe.** It runs and rejects, but never prunes. `gotchas.md` §2, `component-contract.md`, `research/validate-only-probe.md`. |
| 2 | `helpers` differs by segment: `setupPuzzle` (9349) uses `createExtendedHelpers` (9201) with `lines`, `misc` and a region-aware `geometry` (`getSubsetsPerRegion`); `compileCustomComponentClass` (9994) and its `SolverPuzzleView` facade (10072) use plain `createHelpers` (1614). | **Resolved.** `puzzle-api.md`, "Two helpers objects". |
| 3 | `getOrthogonallyConnectedGroups` is a pass-through to `LineGraph.getAllComponents` (494, 345): a generator of `LineGraph` objects, not an array of cell arrays. It lives on `helpers.connectivity`, in the base factory, not on `helpers.misc` as the in-place note below says. | **Resolved.** `puzzle-api.md`, "helpers.connectivity". The generated index's "method" kind is syntactically right and stays. |
| 4 | `bundle-api-index.md` lists 34 components; the bundle has 42 `defineComponent` calls. Cause is not tree-shaking: the generator's filter rejects an array of names (`Pair`/`AsymmetricalPair`, `GreaterThan`/`LessThan`) and a template-literal description (the six group and sum components). | **Ticketed**, sudokumaker-custom-constraints #415. All eight are already in `builtin-components.md`. |
| 5 | `validate` returns a boolean only for custom components. The wrapper converts to `ValidResult` or `{valid: false, message: "unable to satisfy <name>"}` (10061); built-ins return the object with a specific message and opt into `validateDuringSolve` by hand. | **Resolved.** `component-contract.md`, "How `validate` works". |
| 6 | A custom `initialize` runs and then `super.initialize` (10031), whose base (2686) ends in `update`; it never skips the first `update`. | **Resolved.** `component-contract.md`, contract table. |
| 7 | `DigitsHelper` (627) also has `createModuloDigitSet(divisor, remainder)` and `createFilteredDigitSet(predicate)`; the set class has `xor`, and the statics `getUnion(sets)` / `getIntersection(sets)` (608-617) return a fresh set. | **Resolved.** `puzzle-api.md`, `helpers.digits` and DigitSet tables. |
| 8 | `bundle-api-index.md:70` indexes `helpers.geometry` as the base class, so `getSubsetsPerRegion` is absent from it. | **Covered** by item 2's doc; the index cannot see the subclass swap in `createExtendedHelpers`. Left as a note. |
| 9 | `createExtendedHelpers` hands `MiscHelper` the base geometry, not the region-aware one (9218), so `helpers.misc`'s internal geometry has no `getSubsetsPerRegion`. | Bundle quirk, no doc change. Only matters if you reach inside `helpers.misc`. |
| 10 | Straight-ray outer clues from the `BottomRight` and `BottomLeft` corners start at the wrong corner (1183, 1195), and all four corner cases assume a square board. | Bundle bug, no doc change. Do not trust a bottom-corner straight ray. |
