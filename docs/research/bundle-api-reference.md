...
Each is also flagged in place as a `Discrepancy` note.

- **When the solver calls what (SolverState)**: `docs/gotchas.md` §2 says a validate-only component is inert. The bundle shows the call path exists — `SolverState.validate` (9045) calls `validate` on every component with `validateDuringSolve`, and the custom-code wrapper sets that getter to `true` whenever your segment defines `validate` (10050) — independently of whether `update` exists. Nothing in this bundle makes validation conditional on `update`. The observed symptom is real but its cause is not visible here; the safe advice (always ship a real `update`) stands, but the stated mechanism does not match the solver source.
- **How your code becomes a class**: `helpers` is not the same object in the two segments. `setupPuzzle` (9349) builds it with `createExtendedHelpers` (9201), but `compileCustomComponentClass` builds its own with plain `createHelpers` (9994/1614). So the main code's `helpers` has `lines` (LinesHelper), `misc` (MiscHelper) and a region-aware `geometry` (`RegionAwareGeometryHelper`, which adds `getSubsetsPerRegion`), while **inside a component segment `helpers.lines` and `helpers.misc` are `undefined` and `helpers.geometry` lacks `getSubsetsPerRegion`**. `puzzle.helpers` inside `update` is that same reduced object, since `__getFacade` passes it to the `SolverPuzzleView`. `docs/puzzle-api.md` lists `helpers.lines.getLineEnds` without noting it is setup-only. Compute line ends in the main code and pass them as a parameter.
- **helpers.misc (class MiscHelper)**: `docs/puzzle-api.md:64` files `helpers.lines` alongside `helpers.geometry` without saying it is conditional. `LinesHelper` and `MiscHelper` are attached only by `createExtendedHelpers` (`bundle.claude.js:9201`); the base `createHelpers` (`bundle.claude.js:1614`) returns neither, which is why `docs/research/bundle-api-index.md` — generated from the base factory — has sections for `cellIds`, `cornerIds`, `edgeIds`, `outerCellIds`, `geometry` and `connectivity` but none for `lines` or `misc`.
- **helpers.misc (class MiscHelper)**: `docs/research/bundle-api-index.md:158` lists `getOrthogonallyConnectedGroups(arg0)` as a plain "method". It is a pass-through to `LineGraph.getAllComponents`, a **generator** yielding `LineGraph` objects (`bundle.claude.js:478`, `bundle.claude.js:250`), so treating the result as an array of cell arrays fails silently.
- **`helpers.geometry` (class `RegionAwareGeometryHelper extends GeometryHelper`)**: `docs/research/bundle-api-index.md:70` indexes `helpers.geometry` as the base class `gs` and lists only the base members, so `getSubsetsPerRegion` is missing from it. The instance is the region-aware subclass (`bundle.claude.js:9205`).
- **`helpers.geometry` (class `RegionAwareGeometryHelper extends GeometryHelper`)**: `createExtendedHelpers` hands `MiscHelper` the *base* `baseHelpers.geometry`, not the region-aware one it just built (`bundle.claude.js:9218`). `helpers.misc`'s internal geometry therefore has no `getSubsetsPerRegion`. Only matters if you reach into `helpers.misc`.
- **`helpers.geometry` (class `RegionAwareGeometryHelper extends GeometryHelper`)**: (bundle bug, not a doc one): in the straight-ray branch the `BottomRight` case yields `{x: i, y: height-1-i}` — starting at the *bottom-left* corner and walking up-right — and `BottomLeft` yields `{x: width-1-i, y: height-1-i}`, starting at the bottom-right (`bundle.claude.js:1183` and `bundle.claude.js:1195`). The two corners are swapped relative to `TopLeft`/`TopRight`, which are correct. Don't trust a bottom-corner outer clue's straight ray. Also, all four corner cases assume a square board: they run `min(width, height)` steps along the true diagonal, which is not the visual diagonal of a non-square grid.
- **`DiagonalType` (global `DiagonalType`)**: the brief lists `../../bundle-api-index.md` as required reading, but no such file exists in `docs/research/humanify-pedagogy/`. This section was written against `docs/puzzle-api.md` and `docs/component-contract.md` only.
- **`DiagonalType` (global `DiagonalType`)**: `docs/puzzle-api.md:61-62` lists `createFullDigitSet()` and `createOddsDigitSet()` / `createEvensDigitSet()` as **[docs]**, unverified. Both are real, on `DigitsHelper` at `bundle.claude.js:627`, along with two undocumented siblings, `createModuloDigitSet(divisor, remainder)` and `createFilteredDigitSet(predicate)`. All four build within the puzzle's `minDigit`..`maxDigit` range.
- **`DiagonalType` (global `DiagonalType`)**: `docs/puzzle-api.md:103` calls `set.intersect` / `.union` / `.subtract` "Set algebra" that mutates and returns the receiver, which is right, but the table omits `xor`, which behaves identically, and omits the non-mutating statics `SmallNumberSet.getUnion(sets)` and `.getIntersection(sets)` (`bundle.claude.js:608-617`) that do give a fresh set.
- **Built-in components, A to M**: `../../bundle-api-index.md` lists 34 registered components. This bundle contains 42 `defineComponent` calls. The eight the index omits are `Pair`/`AsymmetricalPair`, `DifferentGroups`, `RequiredGroups`, `DiverseGroups`, `GreaterThan`/`LessThan`, `SameGroup`, `SandwichSum` and `WeightedSum`; four of those (`DifferentGroups`, `DiverseGroups`, `GreaterThan`, plus the `Pair` base) fall in this section. The index is generated from the HAR's shipped chunks, so the likely cause is tree-shaking in those chunks, not a naming difference — but treat the four as present and callable only after checking the live build.
- **Built-in components, A to M**: `../../../gotchas.md` §2 says a validate-only custom component is inert. The bundle says otherwise. `SolverState.validate` (`bundle.claude.js:9045`) walks every component with `validateDuringSolve` and calls its `validate` on every state, not only at a leaf — and the custom component wrapper installs `validateDuringSolve` as a getter returning `true` whenever the segment defines a `validate` function (`bundle.claude.js:10050`). A validate-only custom component should therefore reject wrong states; what it cannot do is prune candidates, so the observed symptom in the gotcha (cells keep their full candidate set) is expected while the "entering a wrong value raises no conflict" half is not explained by this code. Worth re-testing before relying on either reading.
- **Built-in components, A to M**: `../../../component-contract.md` states that `validate` returns a boolean. Every built-in returns either the shared `ValidResult` object or `{valid: false, message, cells?}`. Not a doc error: the custom component wrapper converts a truthy return to the shared valid object and a falsy one to a failure whose message is always "unable to satisfy" plus the component name (`bundle.claude.js:10061`). The built-in classes you might hand to `replaceComponent` do not use the boolean convention, and their failure messages are specific where a custom component's is always that generic one.
- **Built-in components, N to Z**: `docs/component-contract.md` says `validate` returns a boolean. That is true only for **custom** components: the compiler wraps a custom `validate` as `validateWrapper() ? __VALID : {valid: false, message: …}` and also force-defines `validateDuringSolve` to `true` on any component that supplies one (`bundle.claude.js:10056`). Built-ins return the result object directly and opt into `validateDuringSolve` by hand.
- **Built-in components, N to Z**: the contract doc describes `initialize` as an optional one-time pass. For custom components the wrapper runs the author's `initialize` and **then** `super.initialize`, which itself calls `update` (`bundle.claude.js:10031`, base at `bundle.claude.js:2686`). A custom `initialize` therefore never skips the first `update`.
...
> (10050) — independently of whether `update` exists. Nothing in this bundle
...
> `LineGraph` objects (`bundle.claude.js:478`, `bundle.claude.js:250`), so
...
> Both are real, on `DigitsHelper` at `bundle.claude.js:627`, along with two
...
> Discrepancy: `docs/puzzle-api.md:103` calls `set.intersect` / `.union` /
...
> `.getIntersection(sets)` (`bundle.claude.js:608-617`) that do give a fresh set.
...
> Discrepancy: `../../../gotchas.md` §2 says a validate-only custom component is
> inert. The bundle says otherwise. `SolverState.validate`
> (`bundle.claude.js:9045`) walks every component with `validateDuringSolve` and
> calls its `validate` on every state, not only at a leaf — and the custom
> component wrapper installs `validateDuringSolve` as a getter returning `true`
> whenever the segment defines a `validate` function (`bundle.claude.js:10050`).
> A validate-only custom component should therefore reject wrong states; what it
> cannot do is prune candidates, so the observed symptom in the gotcha (cells
> keep their full candidate set) is expected while the "entering a wrong value
> raises no conflict" half is not explained by this code. Worth re-testing
> before relying on either reading.
...
> `{valid: false, message, cells?}`. Not a doc error: the custom component
> wrapper converts a truthy return to the shared valid object and a falsy one to
> a failure whose message is always "unable to satisfy" plus the component name
> (`bundle.claude.js:10061`). The built-in classes you might hand to
> `replaceComponent` do not use the boolean convention, and their failure
> messages are specific where a custom component's is always that generic one.
...
> supplies one (`bundle.claude.js:10056`). Built-ins return the result object
...
> (`bundle.claude.js:10031`, base at `bundle.claude.js:2686`). A custom
