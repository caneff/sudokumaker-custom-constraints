...
| `nfa.md` | — | not yet read |
| `sudoku_builder.js` `XSum` case, `_yieldOr`, `_givenHandler`; `handlers.js` `Or`/`And`/`GivenCandidates` runtime state; `sum.md` §2–4 | X-sums as prior art for Up to N | `docs/research/371-xsum-iss-model.md` (#371). Build-time length-branch decomposition, bounds not subset DP; no `Or`/`And` primitive to port, only the case-split idea. |
