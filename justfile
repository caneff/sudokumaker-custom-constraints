...
test:
    ...
    # finders/hunt's own tests (#484, #485, #486): the dedupe key's public
    # function, neighbours and connected components on hand-drawn boards,
    # the text printer's output, the `hunt` CLI run in a subprocess with the
    # toy finder, and the CP-SAT helpers (uniqueness, minimizer, watchdog)
    # on tiny models -- all well under a few seconds, workers pinned to 1.
