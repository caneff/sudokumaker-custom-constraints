...
def groups_w3(r0, c0):
    ...
    for k in R6:
        ...
        gs.append([i(r0 - 1, c), i(r0, c), i(r0 + 1, c), i(r0 + 2, c)])
        gs.append([i(r0 + 6, c), i(r0 + 5, c), i(r0 + 4, c), i(r0 + 3, c)])
    return gs
...
def build(assign, fixed=None):
    m = cp_model.CpModel()
    need = set()
    gsets = {}
    for color, o in GRIDS.items():
        gsets[color] = groups_w3(*o) if assign[color] == "out" else groups(*o, False)
    for cs in HOUSES: need.update(cs)
    for gs in gsets.values():
        for cs in gs: need.update(cs)
    x = {c: m.NewIntVar(1, 6, f"x{c}") for c in need}
...
REPS = [  # one per symmetry class; class 3 is the shipped board's placement
    ("class1 sky/nr vertical, rs/out horizontal", {"Red": "sky", "Green": "nr", "Purple": "rs", "Blue": "out"}),
    ("class2 rs/out vertical, sky/nr horizontal", {"Red": "rs", "Green": "out", "Purple": "sky", "Blue": "nr"}),
    ("class3 CURRENT sky/out vertical, nr/rs horizontal", {"Red": "sky", "Green": "out", "Purple": "rs", "Blue": "nr"}),
    ("class4 nr/rs vertical, sky/out horizontal", {"Red": "rs", "Green": "nr", "Purple": "sky", "Blue": "out"}),
]
for label, assign in REPS:
    m, x, need = build(assign)
    ...
    if st not in ("OPTIMAL", "FEASIBLE"):
        ...
        continue
    cand = {c: {s.Value(x[c])} for c in need}
    probes = 0
    for c in sorted(need):
        for d in range(1, 7):
            if d in cand[c]: continue
            m2, x2, _ = build(assign, (c, d))
            s2 = cp_model.CpSolver(); s2.parameters.max_time_in_seconds = 30; s2.parameters.num_workers = 8
            st2 = s2.StatusName(s2.Solve(m2)); probes += 1
            if st2 in ("OPTIMAL", "FEASIBLE"):
                for c3 in need: cand[c3].add(s2.Value(x2[c3]))
            elif st2 != "INFEASIBLE":
                raise RuntimeError("unknown probe")
    forced = {c: min(cand[c]) for c in need if len(cand[c]) == 1}
    print(f"== {label} == probes {probes} forced {len(forced)} of {len(need)}", flush=True)
    for r in range(20):
        row = ""
        for c in range(20):
            i2 = r * 20 + c
            row += str(forced[i2]) if i2 in forced else ("." if i2 in need else " ")
        print(row)
